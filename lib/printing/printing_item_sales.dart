...
class PrintingItemSalesPage extends StatefulWidget{
  final Map<String, dynamic> itemSales;
  final String formattedTime;

  PrintingItemSalesPage(this.itemSales, this.formattedTime);

  @override
  _PrintingItemSalesPageState createState() => _PrintingItemSalesPageState();
}

class _PrintingItemSalesPageState extends State<PrintingItemSalesPage>{
  ...
  Future<List<Map<String, dynamic>>> fetchDailyItemSales(String totalSalesDay) async {
    final data = await DatabaseHelper.getDailyItemsSold(totalSalesDay);
    return List<Map<String, dynamic>>.from(data);
  }

  Future<List<Map<String, dynamic>>> fetchMonthlyItemSales(String totalSalesMonth) async {
    final data = await DatabaseHelper.getMonthlyItemsSold(totalSalesMonth);
    return List<Map<String, dynamic>>.from(data);
  }
  ...
  Future<List<int>> getTicket() async {
    ...
    if (widget.itemSales['total_sales_month'] != null) {
      items = await fetchMonthlyItemSales(widget.itemSales['total_sales_month']);
    } else {
      items = await fetchDailyItemSales(widget.itemSales['total_sales_day']);
    }
    ...
    String reportTitle = widget.itemSales['total_sales_day'] != null ? 'Daily Item Sales' : 'Monthly Item Sales';
    ...
    // sortedItems.sort((a, b) => b['no_item'].compareTo(a['no_item']));
    ...
    for (var item in items) {
      ...
      List<PosColumn> itemColumns = [
        ...
        PosColumn(
          text: item['sum_no_item'].toString(),
          width: 2,
          styles: const PosStyles(align: PosAlign.right),
        ),
        PosColumn(
          text: item['sum_total'].toString(),
          width: 3,
          styles: const PosStyles(align: PosAlign.right),
        ),
      ];
    ...
    List<PosColumn> itemColumns = [
      ...
      PosColumn(
        text: widget.itemSales['gross_sales'],
        width: 3,
        styles: const PosStyles(align: PosAlign.right),
      ),
    ];
          ...
          children: [
            ...
            SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: const Text("Print Item Sales", style: TextStyle(fontSize: 24.0),),
                  style: ElevatedButton.styleFrom(
                    primary: Colors.green[400], // Set the button background color
                  ),
                )
            ),
          ],
